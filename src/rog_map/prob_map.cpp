...
void ProbMap::updateProbMap(const PointCloud& cloud, const Pose& pose) {
    ...
    const Eigen::Quaterniond& quat = pose.second;
...
void ProbMap::raycastProcess(const PointCloud& input_cloud, const Vec3f& cur_odom, const Eigen::Quaterniond& quat) {
    Eigen::Quaterniond quat_normalized = quat.normalized();
    ...
    int temperol_cnt{0};
    for (const auto& pcl_p : input_cloud.points) {
        ...
        // Apply rotation from quaternion and translation from cur_odom to transform the point
        p = quat_normalized * p + cur_odom;
...
void savePLY(const pcl::PointCloud<pcl::PointXYZRGB>::Ptr& cloud, const std::string& filename) {
...
void ProbMap::indexToPos(int index, Vec3f& pos){
    int z = index / (sc_.map_size_i.x() * sc_.map_size_i.y());
    int y = (index / sc_.map_size_i.x()) % sc_.map_size_i.y();
    int x = index % sc_.map_size_i.x();

    // Calculate the world coordinates
    pos.x() = cfg_.fix_map_origin.x() + x * sc_.resolution;
    pos.y() = cfg_.fix_map_origin.y() + y * sc_.resolution;
    pos.z() = cfg_.fix_map_origin.z() + z * sc_.resolution;

}

pcl::RGB interpolateColor(float value, float min_val, float max_val) {
    pcl::RGB color;
    ...
    // Color mapping: green (free) to red (occupied)
    color.r = static_cast<uint8_t>(255 * normalized); // More occupancy, more red
    color.g = static_cast<uint8_t>(255 * (1.0 - normalized)); // Less occupancy, more green
    color.b = 0; // No blue component

    return color;
}

void ProbMap::initVisualization() {
    // Create the PCLVisualizer object
    viewer = pcl::make_shared<pcl::visualization::PCLVisualizer>("Map Visualization");
    viewer->addCoordinateSystem(1.0);
    viewer->setBackgroundColor(0, 0, 0);

    // Optionally, set camera parameters
    viewer->initCameraParameters();
}

void ProbMap::visualizeMap() {
    std::cout << "mapsize: " << sc_.map_size_i << endl;
    ...
    for(int i = 0; i < occupancy_buffer_.size(); ++i){
        if(occupancy_buffer_[i] > maxVal)
            maxVal = occupancy_buffer_[i];
        if(occupancy_buffer_[i] < minVal)
            minVal = occupancy_buffer_[i];

    }

    std::cout<<"minmax:"<<minVal << "," << maxVal << endl;
    std::cout << "cfg lmax: " << cfg_.l_max << endl;

    for (int i = 0; i < occupancy_buffer_.size(); ++i) {
        Vec3f pos;
        // Convert index to position (example function, implement accordingly)
        hashIdToPos(i, pos);
        ...
        point.y = pos.y();
        point.z = pos.z();
        float occupancy = occupancy_buffer_[i];
        if (occupancy > cfg_.l_occ) {
            //pcl::RGB color = interpolateColor(occupancy, cfg_.l_occ, 80);
            point.r = 255; point.g = 0; point.b = 0;
            cloud->push_back(point);} // Red for occupied
        // } else if (occupancy < cfg_.l_free) {
        //     point.r = 0; point.g = 255; point.b = 0; // Green for free
        // } else {
        //     point.r = 255; point.g = 255; point.b = 255; // White for unknown
        // }
        
        
    }

    savePLY(cloud, "output.ply");

    pcl::visualization::PCLVisualizer viewer("Map Visualization");
    viewer.addCoordinateSystem(1.0);
    viewer.setBackgroundColor(0, 0, 0);
    viewer.addPointCloud<pcl::PointXYZRGB>(cloud, "map cloud");
    viewer.setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 2, "map cloud");
...
void ProbMap::updateVisualization() {
    // Generate the point cloud from your occupancy buffer
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud(new pcl::PointCloud<pcl::PointXYZRGB>);

    // Populate 'cloud' with points from the occupancy buffer
    for (size_t i = 0; i < occupancy_buffer_.size(); ++i) {
        float occupancy = occupancy_buffer_[i];

        // Decide whether to visualize this point based on occupancy value
        if (occupancy > cfg_.l_occ) { // Adjust the threshold as needed
            Vec3f pos;
    ...
    if (!viewer->updatePointCloud(cloud, "map cloud")) {
        viewer->addPointCloud<pcl::PointXYZRGB>(cloud, "map cloud");
